...
public class ButtonListener implements OnTouchListener {
    ...
    public boolean onTouch(final View v, MotionEvent event) {
		...
		if (event.getActionMasked() == MotionEvent.ACTION_UP) {
			...
			time.setText(activity.getString(R.string.remainingTimeText, chargedTime));

            new CountDownTimer(TimeUnit.SECONDS.toMillis(chargedTime), TimeUnit.SECONDS.toMillis(1)) {
				...
				public void onFinish() {
					...
					if (((CheckBox) activity.findViewById(R.id.soundBox)).isChecked()) {
						...
						MediaPlayer mp = MediaPlayer.create(activity, notification);
						mp.start();
					}
				}
